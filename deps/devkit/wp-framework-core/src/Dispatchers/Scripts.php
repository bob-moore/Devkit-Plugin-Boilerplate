 ...
 * @internal
 */
class Scripts extends Abstracts\Mountable implements Interfaces\Dispatchers\Scripts, Interfaces\Handlers\Directory, Interfaces\Handlers\Url
{
    ...
    public function register(string $handle, string $path, array $dependencies = [], string $version = '', $in_footer = \true) : string
    {
         ...
         * Enqueue script
         */
        \wp_register_script($handle, $path, \apply_filters("{$this->package}_{$handle}_script_dependencies", $dependencies), $version, $in_footer);
        return $handle;
    }
}
