...
class ContainerBuilder extends \Devkit\Plugin\Deps\DI\ContainerBuilder
{
    ...
    protected function autowireControllers(string|array|DefinitionSource|Helper\DefinitionHelper $definition, string $key = '') : array
    {
        ...
        } elseif (\is_object($definition) && \is_subclass_of($definition, Helper\DefinitionHelper::class) && !empty($key)) {
            $definition_object = $definition->getDefinition($key);
            if (\is_object($definition_object) && \method_exists($definition_object, 'getClassName')) {
