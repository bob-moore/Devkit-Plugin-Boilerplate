...
class Compiler extends Abstracts\Mountable implements Interfaces\Services\Compiler, Interfaces\Handlers\Directory
{
    ...
    public function templateLocations(array $locations) : array
    {
        if (empty($this->template_locations)) {
            ...
            $this->views_dir = \apply_filters("{$this->package}_views_directory", $this->views_dir);
            $package_template_directories = [\trailingslashit(\get_stylesheet_directory() . '/' . $this->views_dir), \trailingslashit(\get_template_directory() . '/' . $this->views_dir), \trailingslashit($this->dir($this->assets_dir)), \trailingslashit($this->dir($this->views_dir))];
            $package_template_directories = \apply_filters("{$this->package}_views_directories", $package_template_directories);
            $this->template_locations[$this->package] = \array_unique(\array_filter($package_template_directories, function ($dir) {
                return \is_dir($dir);
            }));
        }
    ...
    protected function filterTemplateLocations(string|array $location) : bool|array|string
    {
        if (\is_array($location)) {
            $filtered_locations = \array_filter($location, function ($template_location) {
    ...
    {
        return \did_filter("{$this->package}_twig_functions") ? $this->functions : \apply_filters("{$this->package}_twig_functions", $this->functions);
    }
    /**
    ...
    {
        return \did_filter("{$this->package}_twig_filters") ? $this->filters : \apply_filters("{$this->package}_twig_filters", $this->filters);
    }
    /**
    ...
    {
        $this->functions[$name] = ['callback' => $callback, 'args' => $args];
    }
    /**
    ...
    {
        $this->filters[$name] = ['callback' => $callback, 'args' => $args];
    }
}
